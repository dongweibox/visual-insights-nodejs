 ...
 */
/* global $, Prism, Viz, personImagePreview, _*/
...
$(document).ready(function() {
  ...
  $.getJSON('data/persons.json', function(data) {
    ...
    $person1Images.append(_.template(template, {
      items: person1
    }));
    $person2Images.append(_.template(template, {
      items: person2
    }));
  });
  ...
  function getSummary() {
    xhr = $.post('/summary', $form.serialize(), function(response) {
        ...
        $error.hide();
        $('html, body').animate({
          scrollTop: $('.output--header').offset().top
        }, 600);
